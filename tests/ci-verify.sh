...
if grep -qi 'zfs.*"jail"' "$BUILDDIR/run.cpp" && grep -qi 'zfs.*"unjail"' "$BUILDDIR/run.cpp"; then
...
if grep -qi 'execCommandGetStatus.*jexec' "$BUILDDIR/run.cpp"; then
...
if grep -qi 'rctl' "$BUILDDIR/run.cpp"; then
...
if grep -qi 'zfs.*snapshot.*snapName\|zfs.*clone.*cloneName' "$BUILDDIR/run.cpp"; then
