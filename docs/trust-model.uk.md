...
**Стосується:** 1.1.11 (rootless-модель, `crate(1)` без setuid). Про
≤ 0.9.x setuid-модель і міграцію див.
...
Міркувати про ізоляцію на 1.1.11 — це міркувати про те, хто має доступ
...
- **libnv `AF_UNIX` сокет (0.9.14).** Gated по групі: лістенер робить
  ...
  (`daemon/privops_listener.cpp:90`) — але лише для гачка per-user
  audit / namespacing, **не** для авторизації.

**Авторизації за ресурсом немає на жодному транспорті.** Диспетчеризація
вербів — це `parse → validate → handle` (`dispatchPrivOp`,
`daemon/privops_handlers.cpp:651`; `dispatchPrivOpFromMap`, `:907`).
Обробники **сліпі до uid** — `handleCreateJail(const CreateJailReq &)`
(`:332`), `handleAttachZfs` (`:93`), `handleSetRctl` (`:41`) діють суто
за аргументами запиту. uid піра споживається лише *після* операції,
best-effort side-effect'ами: `maybeWritePerUserAudit` (`:602`) і
`maybeApplyUmbrella` (`:630`). Жодна pool-ACL / ownership-перевірка не
виконується *перед* виконанням верба.

> Наслідок: будь-хто, хто має доступ до privops — `admin` bearer-токен
> або членство в групі privops-сокета — може створити, знищити,
> просигналити чи перемонтувати (ZFS/mount/RCTL) **будь-який** jail на
> хості. Це єдиний домен довіри, точно як старий setuid `crate(1)`.
> Видати оператору доступ до privops = видати йому root по всьому хосту.
...
Площина 1 (privops) сьогодні свідомо є єдиним доменом довіри: host-wide
`admin`-токен на HTTP, host-wide членство в групі на libnv-сокеті. Це
прийнятно **доки доступ до privops трактується як еквівалент видачі
старого setuid `crate(1)`** — тобто видається лише цілком довіреним
операторам.
...
privops безпечним для **взаємно недовірливих** операторів, мусить
виконуватись таке — інакше per-user-розділення є гігієною для чесних
операторів, а не межею безпеки:

1. **Авторизуй до диспетчеризації.** uid/gid від `getpeereid`
   *ідентифікує* викликача; він **не** *авторизує* операцію. Кожен
   привілейований верб має проганяти pool-ACL / ownership-перевірку — тієї
   ж форми, що `poolVisibleOnSocket` / `tokenAllowsContainer` — прив'язану
   до **цільового** jail/пулу/датасета, *перед* виконанням операції.
   Сьогодні uid піра живить лише audit-хвіст; верб виконується попри це.
...
2. **Пер-операторний namespacing — це зручність, а не межа.** Будь-який
   ...
   uid-префікса викликача — ніколи не братись на віру, що саме й роблять
   сьогодні uid-сліпі обробники.
...
Доки (1)–(2) не зроблені, мультитенантне розгортання, де оператори мають
самі створювати jail'и, має посередничати створення jail через довірений
брокер, а не видавати операторам сирий доступ до privops-сокета.
