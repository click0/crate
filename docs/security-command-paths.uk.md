...
- [`trust-model.uk.md`](trust-model.uk.md) — де саме enforce-иться (і де
  НЕ enforce-иться) мультитенантна ізоляція між privops- та pooled-
  control-площинами. Описане тут загартування шляхів команд захищає
  *довірений* привілейований процес від ворожого оточення — історично
  setuid `crate(1)`, а з 1.0.0 — root-демон `crated(8)`, що запускає ці
  host-інструменти (див. [`rootless-migration.md`](rootless-migration.md));
  воно не відокремлює операторів один від одного.
